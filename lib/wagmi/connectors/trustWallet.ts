...
export function getTrustWalletProvider(): WindowProvider | undefined {
  const isTrustWallet = (ethereum: NonNullable<Window['ethereum']>) => {
    ...
    const trustWallet = !!(ethereum as WindowProvider).isTrust
  ...
  if (isTrustWallet(window.ethereum as NonNullable<Window['ethereum']>)) {
    return window.ethereum as WindowProvider
  }

  // Trust Wallet provider might be replaced by another
  // injected provider, check the providers array.
  if ((window?.ethereum as WindowProvider)?.providers) {
    return (window?.ethereum as WindowProvider)?.providers?.find(isTrustWallet)
  }
